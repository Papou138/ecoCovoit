/**
 * Gestion de la page de détails d'un covoiturage
 * US5 & US6 - Vue détaillée et participation
 */
...
class TripDetailManager {
    ...
    initEventListeners() {
        ...
        this.participateBtn.addEventListener('click', () => this.handleParticipation());

        // Modal
        this.modalClose.addEventListener('click', () => this.closeModal());
        this.cancelBtn.addEventListener('click', () => this.closeModal());
        this.confirmBtn.addEventListener('click', () => this.confirmParticipation());
    ...
    displayTripDetails(trip) {
        // Route et timing
        document.getElementById('departure-city').textContent = trip.route.departure;
        document.getElementById('arrival-city').textContent = trip.route.arrival;
        document.getElementById('departure-time').textContent = trip.route.departureTime;
        document.getElementById('arrival-time').textContent = trip.route.arrivalTime;
        ...
        // Places et prix
        document.getElementById('available-seats').textContent = `${trip.availableSeats} sur ${trip.totalSeats}`;
        document.getElementById('trip-price').textContent = `${trip.price}€`;
        document.getElementById('total-price').textContent = `${trip.price}€`;

        // Véhicule
        document.getElementById('vehicle-name').textContent = `${trip.vehicle.brand} ${trip.vehicle.model}`;
        ...
        energyBadge.className = `energy-badge ${trip.vehicle.isElectric ? 'electric' : 'fuel'}`;
        energyBadge.innerHTML = `
            <i class="fas fa-${trip.vehicle.isElectric ? 'bolt' : 'gas-pump'}"></i>
            ${trip.vehicle.energy}
        `;
        ...
        document.getElementById('driver-rating-value').textContent = trip.driver.rating;
        document.getElementById('driver-rating-count').textContent = `(${trip.driver.reviewCount} avis)`;

        // Étoiles du conducteur
        this.displayStars('driver-stars', trip.driver.rating);

        // Préférences
        const preferencesList = document.getElementById('preferences-list');
        preferencesList.innerHTML = trip.preferences.map(pref => `
    ...
    checkAvailability(trip) {
        ...
        if (!isLoggedIn) {
            this.participateBtn.innerHTML = '<i class="fas fa-sign-in-alt"></i>Se connecter pour participer';
        ...
        if (this.userCredits < trip.price) {
            this.participateBtn.innerHTML = '<i class="fas fa-coins"></i>Crédits insuffisants';
            this.participateBtn.disabled = true;
            this.participateBtn.classList.add('btn-disabled');
    ...
    handleParticipation() {
        ...
        document.getElementById('modal-route').textContent =
            `${this.currentTrip.route.departure} → ${this.currentTrip.route.arrival}`;
        ...
        document.getElementById('modal-datetime').textContent =
            `${formattedDate} à ${this.currentTrip.route.departureTime}`;

        document.getElementById('modal-driver').textContent = this.currentTrip.driver.name;
        document.getElementById('modal-price').textContent = `${this.currentTrip.price}€`;
        document.getElementById('modal-credit-cost').textContent = this.currentTrip.price;

        const remainingCredits = this.userCredits - this.currentTrip.price;
        document.getElementById('modal-remaining-credits').textContent = remainingCredits;

        // Styling des crédits restants
    ...
    async confirmParticipation() {
        try {
            ...
            this.confirmBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>Traitement...';
            ...
            if (result.success) {
                ...
                document.getElementById('available-seats').textContent =
                    `${this.currentTrip.availableSeats} sur ${this.currentTrip.totalSeats}`;
            ...
            this.confirmBtn.innerHTML = '<i class="fas fa-check"></i>Confirmer ma participation';
        }
    }
                ...
                const mockReviews = [
                    {
                        id: 1,
                        rating: 5,
                        comment: "Excellent conducteur ! Très ponctuel et conduite souple. Je recommande vivement !",
                        date: "2025-06-28",
                    ...
                    },
                    {
                        id: 2,
                        rating: 4,
                        comment: "Trajet agréable, conductrice sympathique. Voiture propre et confortable.",
                        date: "2025-06-25",
                    ...
                    },
                    {
                        id: 3,
                        rating: 5,
                        comment: "Parfait ! Communication excellente et respect des horaires. À refaire !",
                        date: "2025-06-20",
    ...
    displayReviews(reviews) {
        ...
        document.getElementById('total-reviews').textContent = `Basé sur ${reviews.length} avis`;
        ...
        reviewsList.innerHTML = displayedReviews.map(review => `
    ...
    loadMoreReviews() {
        // Charger plus d'avis - à implémenter
