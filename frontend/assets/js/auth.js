...
class AuthManager {
    ...
    initEventListeners() {
        ...
        this.passwordConfirmField.addEventListener('input', () => this.checkPasswordMatch());
    ...
    async handleLogin(e) {
        ...
        if (!this.validateEmail(email)) {
            this.showMessage(this.loginMessage, 'Veuillez entrer un email valide', 'error');
            return;
        }

        if (password.length < 3) {
            this.showMessage(this.loginMessage, 'Le mot de passe est requis', 'error');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                this.showMessage(this.loginMessage, 'Connexion réussie ! Redirection...', 'success');

                // Redirection
                setTimeout(() => {
                    const redirectUrl = localStorage.getItem('redirectAfterLogin') || 'user-profile.html';
                    localStorage.removeItem('redirectAfterLogin');
                    window.location.href = redirectUrl;
                }, 1500);

            } else {
                this.showMessage(this.loginMessage, result.message || 'Email ou mot de passe incorrect', 'error');
            }

        } catch (error) {
            console.error('Erreur de connexion:', error);
            this.showMessage(this.loginMessage, 'Une erreur est survenue. Veuillez réessayer.', 'error');
        } finally {
    ...
    async handleRegister(e) {
        ...
        // Validations
        if (!this.validateRegisterForm(pseudo, email, password, passwordConfirm, acceptTerms)) {
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                this.showMessage(this.registerMessage, 'Compte créé avec succès ! Vous pouvez maintenant vous connecter.', 'success');

                // Auto-remplir le formulaire de connexion et switcher
                setTimeout(() => {
                    document.getElementById('login-email').value = email;
                    this.switchToLogin();
                    this.showMessage(this.loginMessage, 'Votre compte a été créé avec 20 crédits offerts !', 'info');
                }, 2000);

            } else {
                this.showMessage(this.registerMessage, result.message || 'Erreur lors de la création du compte', 'error');
            }

        } catch (error) {
            console.error('Erreur d\'inscription:', error);
            this.showMessage(this.registerMessage, 'Une erreur est survenue. Veuillez réessayer.', 'error');
        } finally {
    ...
    validateRegisterForm(pseudo, email, password, passwordConfirm, acceptTerms) {
        ...
        if (pseudo.length < 3 || pseudo.length > 20) {
            this.showMessage(this.registerMessage, 'Le pseudo doit contenir entre 3 et 20 caractères', 'error');
            return false;
        }

        if (!/^[a-zA-Z0-9_-]+$/.test(pseudo)) {
            this.showMessage(this.registerMessage, 'Le pseudo ne peut contenir que des lettres, chiffres, tirets et underscores', 'error');
            return false;
        }

        // Email
        if (!this.validateEmail(email)) {
            this.showMessage(this.registerMessage, 'Veuillez entrer un email valide', 'error');
            return false;
        }

        // Mot de passe
        if (!this.isPasswordSecure(password)) {
            this.showMessage(this.registerMessage, 'Le mot de passe ne respecte pas les critères de sécurité', 'error');
            return false;
        }

        // Confirmation mot de passe
        if (password !== passwordConfirm) {
            this.showMessage(this.registerMessage, 'Les mots de passe ne correspondent pas', 'error');
            return false;
        }

        // Conditions d'utilisation
        if (!acceptTerms) {
            this.showMessage(this.registerMessage, 'Vous devez accepter les conditions d\'utilisation', 'error');
            return false;
        }
    ...
    checkPasswordMatch() {
        ...
        if (password === passwordConfirm) {
            ...
            this.passwordMatch.textContent = '✗ Les mots de passe ne correspondent pas';
    ...
    async performLogin(email, password, remember) {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                const mockUsers = [
                    { email: 'marie@example.com', password: 'password123', name: 'Marie D.', credits: 45, role: 'user' },
                    { email: 'admin@ecocovoit.fr', password: 'admin123', name: 'Admin', credits: 999, role: 'admin' },
                    { email: 'employe@ecocovoit.fr', password: 'employe123', name: 'Employé', credits: 100, role: 'employee' }
                ];
    ...
    async performRegister(pseudo, email, password) {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                const existingEmails = ['marie@example.com', 'admin@ecocovoit.fr'];
