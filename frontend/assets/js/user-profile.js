...
document.addEventListener('DOMContentLoaded', function() {
            ...
            userInfoContainer.innerHTML = `
                <div class="user-info">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value">${getRoleLabel(data.user.role)}</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">Membre depuis</span>
                        <span class="info-value">${formatDate(data.user.created_at || '2025-01-01')}</span>
                    </div>
                </div>
            `;
    ...
    async function loadVehicles() {
        try {
            ...
            vehiclesContainer.innerHTML = data.vehicules.map(vehicle => `
    ...
    async function loadPreferences() {
        try {
            ...
            const preferences = [
                { id: 'music', label: 'Musique autorisée', value: true },
                { id: 'smoking', label: 'Fumeur', value: false },
                { id: 'pets', label: 'Animaux acceptés', value: true },
                { id: 'talking', label: 'Discussion', value: true },
                { id: 'air_conditioning', label: 'Climatisation', value: true }
            ];
    ...
    async function loadCurrentTrips() {
        try {
            ...
            tripsContainer.innerHTML = data.trajets.map(trip => `
                <div class="trip-item" data-trip-id="${trip.id}">
                    <div class="trip-route">
                        ...
                        <div class="trip-details">
                            ${formatDateTime(trip.date_depart)} • ${trip.places_disponibles} places • ${trip.prix}€
                        </div>
                    </div>
                    <div class="trip-status ${trip.statut}">${getStatusLabel(trip.statut)}</div>
                    <div class="trip-actions">
                        ...
                        ` : ''}
                        <button class="btn-danger" onclick="cancelTrip(${trip.id})">
    ...
    async function loadStatistics() {
        try {
            ...
            document.getElementById('kilometers-saved').textContent = stats.kilometers_saved;
            document.getElementById('co2-saved').textContent = stats.co2_saved;
            document.getElementById('rating-average').textContent = stats.rating_average.toFixed(1);
    ...
    function setupEventListeners() {
        ...
        document.getElementById('view-all-trips-btn').addEventListener('click', () => {
        ...
        document.getElementById('credits-history-btn').addEventListener('click', () => {
    ...
    window.completeTrip = async function(tripId) {
        ...
        try {
            ...
            if (data.success) {
                ...
                showNotification(data.message || 'Erreur lors de la finalisation', 'error');
            }
